 ...
 */
export function parseOpzoekAntwoord(ruw, kolom) {
    let antwoord;
    try {
 ...
 */
export function kolomVan(issue, cwd) {
...
function issueVeld(issue, jq, cwd) {
    const omgeving = ghOmgeving();
    if (!omgeving.kan) {
        return undefined;
    }
    const ruw = uitvoerMetEnv('gh', ['api', `repos/${EIGENAAR}/${BACKLOG_REPO}/issues/${String(issue)}`, '--jq', jq], cwd, omgeving.env);
 ...
 */
export function parseOuderAntwoord(ruw) {
 ...
 */
export function ouderVan(issue, cwd) {
 ...
 * zodat de contract-tests de interpretatie kunnen vastpinnen.
 */
export function parseKinderenAntwoord(ruw) {
 ...
 */
export function parseOpenKinderen(ruw) {
    const [gedaan, totaal] = ruw.trim().split('/').map(Number);
 ...
 */
export function alleKinderenDicht(ouder, cwd) {
 ...
 */
export function openKinderenAantal(issue, cwd) {
...
export function sluitIssue(issue, cwd) {
    const omgeving = ghOmgeving();
    if (!omgeving.kan) {
        return;
    }
    const uitkomst = run('gh', ['issue', 'close', String(issue), '--repo', `${EIGENAAR}/${BACKLOG_REPO}`], {
        ...(cwd === undefined ? {} : { cwd }),
        ...(omgeving.env === undefined ? {} : { env: omgeving.env }),
        capture: true,
        toleranter: true,
...
export function zetLabel(issue, label, cwd) {
    ...
    const uitkomst = run('gh', ['issue', 'edit', String(issue), '--repo', `${EIGENAAR}/${BACKLOG_REPO}`, '--add-label', label], {
        ...(cwd === undefined ? {} : { cwd }),
        ...(omgeving.env === undefined ? {} : { env: omgeving.env }),
        capture: true,
        toleranter: true,
...
export function verwijderLabel(issue, label, cwd) {
    const omgeving = ghOmgeving();
    if (!omgeving.kan) {
        return;
    }
...
export function schrijfBody(issue, bodyBestand, cwd) {
    const omgeving = ghOmgeving();
    if (!omgeving.kan) {
        return false;
    }
...
export function zetItemsUitBereikOpDone(vanaf, tag, itemMelding, ouderMelding, cwd) {
    ...
    for (const issue of issues) {
        ...
        if (aantalOpen > 0) {
            waarschuwing(`#${String(issue)} heeft nog ${String(aantalOpen)} open ` +
 ...
 */
export function orkestratorComments(issue, markering, cwd) {
    const omgeving = ghOmgeving();
    if (!omgeving.kan) {
        return [];
    }
