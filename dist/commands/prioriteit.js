import { bordItems, escalaties, zetPrioriteit } from '../board.js';
...
export function prioriteit(issueArgument, waardeArgument) {
    ...
    const geblokkeerd = escalaties(cwd) ?? new Set();
    const refine = items.filter((item) => item.kolom === 'Klaar voor technische refinement' && !geblokkeerd.has(item.issue));
    const bouw = items.filter((item) => item.kolom === 'Klaar voor Bouwen' && !geblokkeerd.has(item.issue));
    toonRij('Refine-wachtrij', refine);
    toonRij('Bouw-wachtrij', bouw);
}
function toonRij(titel, items) {
...
function vereisIssue(waarde) {
    ...
    if (!Number.isSafeInteger(issue) || issue <= 0) {
        throw new GebruikersFout('Gebruik: factory prioriteit <issuenummer> [getal]\n' +
...
//# sourceMappingURL=prioriteit.js.map