...
async function bouwAf(item, cwd, wortel, budgetUsd, reviewBudgetUsd, effort, leverIn, apps = [], reeks, env, timeoutMs) {
    ...
    try {
        ...
        if (fout instanceof OmgevingsFout) {
            ...
            // maken heeft (#383).
            blokkeer(item, cwd);
            plaatsComment(item.issue, `**Omgevingsfout.** De bouw-run kon niet starten.\n\n` +
                `Fout: ${fout.message}\n` +
                `Pad: ${werkmap}\n\n` +
...
function verwerkBouw(item, uitkomst, reviewUitkomst, cwd, wortel, leverIn, reeks) {
    ...
    try {
        ...
        if (fout instanceof OmgevingsFout) {
            ...
            blokkeer(item, cwd);
            plaatsComment(item.issue, `**Omgevingsfout bij het inleveren.** De kwaliteitspoort kon niet draaien.\n\n` +
                `Fout: ${fout.message}\n` +
                `Pad: ${werkmap}\n\n` +
