...
import { AGENT_REFINER, draaiWerker, formatDoorloop, stilOpgelostPunten, } from '../werker.js';
 ...
 */
export function appSpiegels(wortel) {
    ...
    return readdirSync(wortel, { withFileTypes: true })
        ...
        if (!entry.isDirectory())
            return false;
        if (entry.name === 'factory')
            return false;
        if (entry.name.endsWith('-wt') || entry.name.endsWith('-bron'))
            return false;
 ...
 */
export function escalatieComment(issue, vraag, advies, uitkomst, werkmap, soort = 'refine', app, doorloop) {
    const doorloopTabel = doorloop !== undefined && doorloop.length > 0 ? `\n\n${formatDoorloop(doorloop)}\n` : '';
    return (`**Escalatie.**\n\n` +
        `${VRAAG_MERK}\n**Vraag:** ${vraag}\n${VRAAG_EIND}\n\n` +
        `${ADVIES_MERK}\n**Advies:** ${advies}\n${ADVIES_EIND}\n\n` +
 ...
 */
export function heeftWachtOp(body) {
...
function rondAf(issue, body, samenvatting, slices, uitkomst, werkmap, cwd, keuzeNotitie) {
    ...
    const doelKolom = heeftWachtOp(body) ? 'Wacht op akkoord' : 'Klaar voor Bouwen';
...
export function orkestreerStatus(cwd, opties = {}) {
    ...
    const wachtOpAkkKolom = items.filter((item) => item.kolom === 'Wacht op akkoord' && !geblokkeerd.has(item.issue));
 ...
 */
function verwerkAntwoord(issue, tekst, escalatie, app, uitkomst, cwd) {
    ...
    if (uitkomst.afloop === 'mislukt') {
        plaatsComment(issue, `**Antwoord verwerkt, maar de run mislukte.** ${uitkomst.fout ?? 'onbekende fout'}\n\n` +
            voetnoot(uitkomst, escalatie.werkmap, 'refine', app), cwd);
    ...
    if (verdict?.uitkomst === 'escalatie') {
        ...
        plaatsComment(issue, escalatieComment(issue, verdict.vraag, verdict.advies, uitkomst, escalatie.werkmap, 'refine', app), cwd);
    ...
    return rondAf(issue, verdict.body, verdict.samenvatting, verdict.slices, uitkomst, escalatie.werkmap, cwd);
}
/**
 ...
 */
export function orkestreerWrijving(n, logPad) {
    ...
    if (perTool.size === 0) {
        ...
        for (const [tool, { totaal, runs }] of gesorteerd) {
            ...
            process.stdout.write(`  ${tool}: ${String(totaal)}× in ${String(runs)} run${runs === 1 ? '' : 's'}${markering}\n`);
        }
    }
    // Totalen.
    process.stdout.write('\n');
    ...
    process.stdout.write(` · met wrijving: ${String(runsMetWrijving)} · zonder: ${String(selectie.length - runsMetWrijving)}\n`);
}
