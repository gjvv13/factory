 ...
 */
function controleerIntegriteit(bestand, waar) {
    const integriteit = uitvoerVan('sqlite3', [bestand, 'PRAGMA integrity_check']);
    if (integriteit !== 'ok') {
        throw new GebruikersFout(`Backup ${waar} maar de integriteitscheck faalde: ${integriteit ?? 'geen antwoord'}`);
    }
}
/**
 ...
 */
function roteer(dir, voorvoegsel, bewaar) {
 ...
 */
function kopieerOffsite(offsiteDir, bron, bestandsnaam, voorvoegsel, bewaar) {
    kop('Off-site kopie');
