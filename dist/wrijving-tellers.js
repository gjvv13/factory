 ...
 */
export function leesTellers(pad) {
    if (!existsSync(pad)) {
        return {};
    }
    try {
        const inhoud = readFileSync(pad, 'utf8');
        const parsed = JSON.parse(inhoud);
        if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
            return {};
        }
        // Valideer dat alle waarden getallen zijn; negeer ongeldige entries.
        const resultaat = {};
        for (const [sleutel, waarde] of Object.entries(parsed)) {
 ...
 */
export function werkTellersBij(pad, labels, patroonVan) {
    if (labels.length === 0)
        return [];
...
//# sourceMappingURL=wrijving-tellers.js.map