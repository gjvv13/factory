 ...
 */
export function vergelijkSleutels(
  appDir: string,
  omgeving: 'acc' | 'prod',
 ...
 */
export function leesSleutelContract(repoDir: string): SleutelContract | undefined {
 ...
 */
export function toetsConfigSleutels(
  repoDir: string,
  ...
  for (const r of resultaten) {
    ...
    if (r.nietControleerbaar > 0) {
      waarschuwing(
        `${r.omgeving}: ${String(r.nietControleerbaar)} geheime sleutel${r.nietControleerbaar === 1 ? '' : 's'} niet controleerbaar (${r.omgeving}.secrets.env ontbreekt)`,
      );
    }
  ...
  if (stand === 'blokkeer') {
    throw new GebruikersFout(
      `${String(fouten.length)} ontbrekende config-sleutel${fouten.length === 1 ? '' : 's'}. Vul de env-bestanden aan.`,
    );
  }
}
