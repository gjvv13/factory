...
export function parseReleasePrList(json: string): readonly ReleasePr[] {
  const items: unknown = JSON.parse(json);
  if (!Array.isArray(items)) return [];
  return items.filter(
    (item): item is ReleasePr =>
      typeof item === 'object' &&
      item !== null &&
      'number' in item &&
      'headRefName' in item &&
...
function issueUitBranch(branch: string): number | undefined {
...
export function opruimen(opties: OpruimOpties = {}): void {
  ...
  const lokaalVerwijderen: string[] = [];
  const lokaalBlijven: string[] = [];
  const lokaalWorktree: string[] = [];
  const lokaalKonNietChecken: string[] = [];

  for (const branch of lokaal) {
    if (branch === 'main' || branch === huidig) continue;
  ...
  const remoteVerwijderen: string[] = [];
  const remoteBlijven: string[] = [];
  const remoteKonNietChecken: string[] = [];
  ...
  if (blijven.length > 0) {
    waarschuwing(
      `${String(blijven.length)} blijven staan: ${blijven.join(', ')}\n  (niet gemerged)`,
    );
  }
  ...
  if (konNietChecken.length > 0) {
    waarschuwing(
      `${String(konNietChecken.length)} kon niet checken: ${konNietChecken.join(', ')}\n  (PR-state niet op te vragen)`,
    );
  }
  ...
  for (const r of releaseResultaten) {
    if (r.actie === 'gesloten') {
      ...
      waarschuwing(
        `release-PR #${String(r.nummer)} (${r.branch}) overgeslagen — ${r.reden ?? 'onbekende reden'}`,
      );
    }
  }
...
// ---------------------------------------------------------------------------
// Worktree-opruimen (#421)
// ---------------------------------------------------------------------------

interface WorktreeResultaat {
  readonly pad: string;
  readonly branch: string | undefined;
  readonly actie: 'verwijderd' | 'overgeslagen';
 ...
 */
function categoriseerWorktrees(cwd: string, dry: boolean): readonly WorktreeResultaat[] {
  ...
  const hoofdWorktree =
    uitvoerVan('git', ['rev-parse', '--show-toplevel'], cwd)?.replace(/\/$/, '') ?? cwd;
  const resultaten: WorktreeResultaat[] = [];

  for (const entry of entries) {
    // Hoofd-worktree niet aanraken.
    if (entry.pad === hoofdWorktree) continue;
...
// ---------------------------------------------------------------------------
// Release-PR-opruimen (#421)
// ---------------------------------------------------------------------------

interface ReleasePrResultaat {
  readonly nummer: number;
  readonly branch: string;
  readonly actie: 'gesloten' | 'gerebased' | 'overgeslagen';
  readonly reden?: string;
}
 ...
 */
function afhandelenReleasePrs(cwd: string, dry: boolean): readonly ReleasePrResultaat[] {
  const prJson = uitvoerVan(
    'gh',
    [
      'pr',
      'list',
  ...
  for (const pr of prs) {
    ...
    if (vergelijkVersies(prVersie, tagVersie) < 0) {
      if (!dry) {
        run(
          'gh',
          ['pr', 'close', String(pr.number), '--repo', 'gjvv13/factory', '--delete-branch'],
          {
            cwd,
            capture: true,
          },
 ...
 */
function bepaalConflictBestanden(branch: string, cwd: string): string[] | undefined {
  // Gebruik merge-tree om de conflicten te bepalen zonder de worktree te raken.
  const result = run(
    'git',
    ['merge-tree', '--write-tree', '--no-messages', `origin/main`, branch],
    { cwd, capture: true, toleranter: true },
  );
  // Exit code 1 = conflicten. Stdout bevat de tree hash op de eerste regel,
  // gevolgd door de conflicterende bestanden.
  if (result.code === 0) return []; // Geen conflicten.
 ...
 */
function rebaseReleaseBranch(branch: string, cwd: string): boolean {
...
function isGemerged(ref: string, cwd: string): boolean {
  return (
...
function worktreeBranches(cwd: string): ReadonlySet<string> {
