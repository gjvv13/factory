...
export async function orkestreer(opties: OrkestreerOpties = {}): Promise<void> {
  ...
  const modi = [opties.dry, opties.eenmalig, opties.nacht, opties.reeks !== undefined].filter(
    (modus) => modus === true,
  );
  if (modi.length > 1) {
    // Stil één van de modi kiezen laat iemand denken dat de run gestart is.
    throw new GebruikersFout(
      '--dry, --eenmalig, --reeks en --nacht sluiten elkaar uit; kies er één.',
    );
  }
  if (modi.length === 0) {
    ...
    throw new GebruikersFout(
      'Gebruik: factory orkestreer --dry (tonen), --eenmalig (één item), --reeks <n> (een reeks) of --nacht (tot het dagmaximum).',
    );
  }
 ...
 */
function beschrijfRun(uitkomst: RunUitkomst): RunRegel {
  return {
    uitkomst:
...
async function draaiNacht(
  ...
  if (gestart >= instellingen.dagmaximum) {
    ...
    ok(
      `dagmaximum al bereikt (${String(gestart)}/${String(instellingen.dagmaximum)}); niets gedaan.`,
    );
    return;
  }
...
/** Wat één run kostte en hoe hij afliep — genoeg voor de logregel van `--nacht`. */
interface RunUitkomst {
 ...
 */
export function heeftFunctioneleSecties(body: string): boolean {
 ...
 */
export function escalatieComment(
  issue: number,
  vraag: string,
  advies: string,
  uitkomst: WerkerBasis,
  werkmap: string,
 ...
 */
export function laatsteEscalatie(issue: number, cwd: string): Escalatie | undefined {
 ...
 */
export function leesEscalatie(comment: string): Escalatie | undefined {
  const sessie =
    /<!-- orkestrator:(?:\s+soort=(\S+))?(?:\s+app=(\S+))?\s+sessie=([^\s]+)\s+werkmap=(.+?)\s*-->/.exec(
      comment,
    );
  ...
  const soort = sessie[1] === 'bouw' ? ('bouw' as const) : ('refine' as const);
  return {
    vraag,
    advies,
    sessie: sessie[3],
 ...
 */
function tussen(tekst: string, van: string, tot: string): string | undefined {
...
function verwerk(
  ...
  if (verdict?.uitkomst === 'escalatie') {
    ...
    ok(
      `#${String(item.issue)} geëscaleerd — beantwoorden met: factory orkestreer antwoord ${String(item.issue)} "…"`,
    );
    return 'escalatie';
  }
...
function blokkeer(item: { readonly issue: number }, cwd: string): void {
  zetKolom(item.issue, WACHTRIJ_KOLOM, cwd);
...
function rondAf(
  ...
  if (!geschreven) {
    ...
    plaatsComment(
      issue,
      `**Uitwerking kon niet weggeschreven worden.**\n\n${voetnoot(uitkomst, werkmap)}`,
      cwd,
    );
    return 'mislukt';
  }
 ...
 */
export function orkestreerStatus(
  cwd: string,
  opties: { readonly paden?: OrkestratorPaden } = {},
): void {
...
// --- antwoord: de escalatie beantwoorden -------------------------------------
...
export async function orkestreerAntwoord(
  ...
  if (!Number.isSafeInteger(issue) || issue <= 0 || tekst === undefined || tekst.trim() === '') {
    throw new GebruikersFout(
      'Gebruik: factory orkestreer antwoord <issuenummer> "<jouw antwoord>"',
    );
  }
  ...
  if (escalatie === undefined) {
    throw new GebruikersFout(
      `Geen escalatie gevonden op #${String(issue)}.\n` +
...
export function vervolgPrompt(escalatie: Escalatie, tekst: string): string {
  return (
    `Antwoord op je vraag "${escalatie.vraag}":\n\n${tekst}\n\n` +
 ...
 */
export function eigenVersie(): string {
  try {
...
export function nieuwsteTag(cwd: string): string {
  ...
  if (tag === undefined || tag === '') {
    throw new GebruikersFout(
      'Geen release-tag (v*) gevonden; er is niets om globaal te installeren.',
    );
  }
  return tag;
}
 ...
 */
function installeerAgent(paden: OrkestratorPaden): void {
  const cwd = process.cwd();
  ...
  ok(
    `dagmaximum ${String(instellingen.dagmaximum)}, budget $${String(instellingen.budgetPerRun)} per run (${paden.envPad}).`,
  );
  ...
  ok(
    `geladen; \`factory orkestreer --nacht\` draait elke nacht om ${String(NACHT_UUR).padStart(2, '0')}:00 (log: ${paden.logPad}).`,
  );
}
...
export function vereisNachtModus(bin: string): void {
  ...
  throw new GebruikersFout(
    `De globale factory (${bin}) kent \`orkestreer --nacht\` niet.\n` +
...
function verwijderAgent(paden: OrkestratorPaden): void {
