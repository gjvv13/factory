...
export function orkestreer(opties: OrkestreerOpties = {}): void {
  ...
  if (modi.length === 0) {
    ...
    throw new GebruikersFout(
      'Gebruik: factory orkestreer --dry (tonen), --eenmalig (één item) of --nacht (tot het dagmaximum).',
    );
  }
    ...
    while (gestart < instellingen.dagmaximum) {
      ...
      for (const item of rij) {
        if (gedaan.has(item.issue) && !gemeld.has(item.issue)) {
          ...
          waarschuwing(
            `#${String(item.issue)} staat na zijn run nog in de wachtrij — overgeslagen voor vannacht.`,
          );
        }
      }
      ...
      gestart = metBoekhouding(
        { paden, nu, soort: 'refine', pot: 'nacht', item: eerste },
        () => werkAf(eerste, cwd, wortel, draaiOpties),
        beschrijfRun,
      ).gestart;
 ...
 */
function voetnoot(uitkomst: WerkerUitkomst, werkmap: string): string {
  const delen = [
...
function vervolgPrompt(escalatie: Escalatie, tekst: string): string {
  return (
