...
export function werkplekPad(repoWortel: string, issue: number): string {
 ...
 */
export function repoWortelVan(cwd: string): string {
  const gitDir = uitvoerVan(
    'git',
...
export function werkplekVanSessie(cwd: string): string | undefined {
  const top = uitvoerVan('git', ['rev-parse', '--path-format=absolute', '--show-toplevel'], cwd);
  if (top === undefined || top === '') {
    return undefined;
  }
  return top === repoWortelVan(cwd) ? undefined : top;
}
...
function elders(repoDir: string, branch: string): string | undefined {
  ...
  let huidigPad: string | undefined;
 ...
 */
export function werkplek(issueArgument: string | undefined, opties: WerkplekOpties = {}): void {
  ...
  if (bestaand !== undefined) {
    throw new GebruikersFout(
      `${branch} is al uitgecheckt in ${bestaand}. Werk daar verder, of ruim die werkplek eerst op.`,
    );
  }
