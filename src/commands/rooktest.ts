...
async function voerUit(
  ...
  verwachteStatus: number,
  bevat: string | undefined,
): Promise<RooktestUitkomst> {
    ...
    if (bevat !== undefined) {
      ...
      if (!tekst.includes(bevat)) {
        return {
          goed: false,
          reden:
            `antwoord bevat '${bevat}' niet — controleer de rooktest-configuratie ` +
 ...
 * aanroepen.
 */
export async function rooktest(omgevingArgument: string | undefined): Promise<void> {
  ...
  let laatste: RooktestUitkomst = { goed: false, reden: 'niet uitgevoerd' };
  ...
  throw new GebruikersFout(
    `Rooktest ${omgeving} faalde (${laatste.reden ?? 'onbekende reden'}). ` +
