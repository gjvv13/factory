 ...
 */
export function parseSlices(body: string): Slice[] {
  const regels = body.split('\n');
  const slices: Slice[] = [];
  let huidig: { nummer: number; naam: string; startIndex: number } | undefined;
...
function maakSlice(
  kop: { nummer: number; naam: string; startIndex: number },
  regels: string[],
  eindIndex: number,
): Slice {
 ...
 */
export function herschrijfBody(
  origineel: string,
  kinderen: readonly { issue: number; naam: string }[],
): string {
...
function leesIssue(issue: number): IssueData {
  const ruw = run(
    'gh',
    [
      'issue',
  ...
  let data: { title?: string; body?: string; labels?: { name?: string }[]; url?: string };
  try {
    data = JSON.parse(ruw.stdout) as typeof data;
  } catch {
...
export function parseAppAntwoord(ruw: string): string | undefined {
  let antwoord: AppAntwoord;
  try {
    antwoord = JSON.parse(ruw) as AppAntwoord;
  } catch {
    return undefined;
  }
...
function leesApp(issue: number): string | undefined {
  const uitkomst = run(
    'gh',
    [
      'api',
...
function maakKind(
  ...
  const tmpBestand = path.join(
    tmpdir(),
    `factory-splits-${String(Date.now())}-${String(Math.random()).slice(2, 8)}.md`,
  );
...
function wisKolom(issueUrl: string): void {
  const uitkomst = run(
    'gh',
    [
      'project',
 ...
 */
function titelPrefix(ouderTitel: string): string {
...
export function splits(issueNummer: string | undefined): void {
  ...
  for (const slice of slices) {
    ...
    if (kind === undefined) {
      throw new GebruikersFout(
        `Kon slice ${String(slice.nummer)} niet aanmaken — geen kinderen gemaakt.`,
      );
    }
  ...
  ok(
    `#${String(nummer)} is opgesplitst in ${String(kinderen.length)} slices` +
      ` en is nu een epic.`,
  );
}
