...
const SOORTEN = ['unit', 'contract', 'e2e'] as const;
 ...
 */
function leesFinal(repoDir: string, soort: string): CoverageMap | undefined {
  const bestand = path.join(repoDir, 'coverage', soort, 'coverage-final.json');
  if (!existsSync(bestand)) {
    return undefined;
  }
  const data = JSON.parse(readFileSync(bestand, 'utf8')) as CoverageMapData;
 ...
 */
export function schrijfGecombineerdeDekking(repoDir: string): number | undefined {
  ...
  return gecombineerd.getCoverageSummary().lines.pct;
}
