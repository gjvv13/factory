 ...
 */
export function leesFlagMeta(dir: string): FlagMeta | undefined {
 ...
 */
export function beoordeelFlagVerloop(meta: FlagMeta, vandaag: Date): FlagStatus[] {
  const vandaagStart = new Date(vandaag.getFullYear(), vandaag.getMonth(), vandaag.getDate());
  const statussen: FlagStatus[] = [];
...
export function beschrijfVervalstatus(status: FlagStatus): string {
  if (status.soort === 'permanent') return 'permanent';
  if (status.verlooptOp === undefined || status.dagen === undefined) return '';

  if (status.dagen === 0) return `mocht weg op ${status.verlooptOp} (vandaag)`;
  if (status.dagen > 0) {
 ...
 */
export function toetsFlagVerloop(
  dir: string,
  stand: 'waarschuw' | 'blokkeer',
  vandaag: Date,
): void {
  ...
  if (verlopen.length > 0 && stand === 'blokkeer') {
    ...
    throw new GebruikersFout(
      `${String(verlopen.length)} verlopen feature flag${verlopen.length === 1 ? '' : 's'}: ${namen}. Verwijder de flag${verlopen.length === 1 ? '' : 's'} of pas de datum aan in ${FLAG_META_BESTAND}.`,
    );
  }
}
