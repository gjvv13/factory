 ...
 */
export function bouwMelding(items: readonly FastlaneResultaat[]): string {
  if (items.length === 1) {
    ...
    return (
      `🚀 Fastlane: #${String(item.issue)} (${item.app}) is vannacht met auto-merge ingeleverd.\n` +
      item.prUrl
    );
  }
 ...
 */
export async function stuurOchtendmelding(
  items: readonly FastlaneResultaat[],
  ...
  ) => Promise<boolean> = verstuurMelding,
): Promise<void> {
  if (items.length === 0) {
    return;
  }
  if (notifyUrl === undefined || notifyUrl === '') {
    waarschuwing(
      `${String(items.length)} fastlane-item(s) ingeleverd, maar DEPLOY_NOTIFY_URL is niet gezet — geen ochtendmelding verstuurd.`,
    );
    return;
  }
...
async function verstuurMelding(
  ...
  try {
    ...
    waarschuwing(
      `melding naar ${url} mislukt: ${fout instanceof Error ? fout.message : String(fout)}`,
    );
    return false;
  }
}
