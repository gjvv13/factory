 ...
 */
async function draaiendeProdTag(repoDir: string): Promise<string | undefined> {
 ...
 */
export async function toonMigratieStatus(repoDir: string = process.cwd()): Promise<void> {
  const prodTag = await draaiendeProdTag(repoDir);

  let bereikStart: string | undefined;
  ...
  if (prodTag !== undefined) {
    ...
    if (zoekAppDir(repoDir) !== undefined) {
      process.stderr.write(
        'waarschuwing: kon de draaiende prod-versie niet ophalen via /health; terugval op vorige tag.\n',
      );
    }
  }
