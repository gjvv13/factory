...
describe('inleveren', () => {
  ...
  describe('conflict met main', () => {
    ...
    it('gooit bij een conflict met main en rebaset niet zelf (#558)', () => {
      process.chdir(maakRepo());
      // Sinds dist/ niet meer in git staat, is een botsing altijd een echt bron-conflict:
      // inleveren rebaset niet zelf, maar verwijst de mens naar een handmatige rebase.
      const bepaal: UitkomstBepaler = (aanroep, index) =>
        aanroep.argumenten[0] === 'merge-tree'
          ? { code: 1, stdout: ['92d6da3', 'src/cli.ts', '', 'CONFLICT'].join('\n') }
          : gelukkig(aanroep, index);
      const { uitvoerder, aanroepen } = maakUitvoerderOpnemer(bepaal);
      stelUitvoerderIn(uitvoerder);
